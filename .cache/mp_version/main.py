...
def scene_finetune(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    worker, request_queue, response_queue, edit_cache, index_dataset = setup_guidance_worker_and_dataset(train_cams, output_root_dir, source_image_generator, opt.prompt, opt.source_prompt, 
                                                                            opt.num_inference_steps, opt.initial_skip_steps, main_device, torch.device("cuda:1"), opt.guidance_scale, opt.source_guidance_scale)

    viewpoint_stack = [i for i in index_dataset]
    ...
    request_queue.put(StepRequest(new_transport_steps=4, edited_images=[], ordered_edit_ids=[i for i, _ in prepared_viewpoint_stack]))
    response_queue.get()
    for iteration in range(first_iter, final_iter+1):        
        ...
        gt_image_tensor = gt_images.to(main_device).to(image_tensor.dtype)
