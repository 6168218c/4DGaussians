...
from scene.dataset import FourDGSdataset, FourDGSEditDataset
...
class EditScene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], load_coarse=False,
                 editing=True, override_h=None, override_w=None):
        """b
        :param path: Path to colmap scene main folder.
        """
        self.model_path = args.model_path
        ...
        if load_iteration or editing:
            if (not load_iteration and editing) or load_iteration == -1:
                ...
                self.loaded_iter = load_iteration
            if editing:
                if self.loaded_iter is None and os.path.exists(os.path.join(self.model_path, "point_cloud.ply")):
                    self.loaded_iter = -1
                # final check
                if self.loaded_iter is None:
                    raise ValueError("Could not find a suitable model to load for editing!")
            print("Loading trained model at iteration {}".format(self.loaded_iter))
        ...
        self.cameras_extent = scene_info.nerf_normalization["radius"]
        print("Loading Training Cameras")
        self.train_camera = FourDGSdataset(scene_info.train_cameras, args, dataset_type) if not editing else FourDGSEditDataset(scene_info.train_cameras, args, dataset_type)
        print("Loading Test Cameras")
        self.test_camera = FourDGSdataset(scene_info.test_cameras, args, dataset_type)
        print("Loading Video Cameras")
        self.video_camera = FourDGSdataset(scene_info.video_cameras, args, dataset_type)
        self.train_camera.dataset.set_editing(editing)
