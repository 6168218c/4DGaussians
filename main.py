...
import sys
from scene import EditScene, GaussianModel
...
from guidance import setup_guidance_worker_and_dataset, StepRequest
...
def sample_timewise_viewpoints(dataset, num_poses, frame_length, selection_batch_size=None):
    if selection_batch_size is None:
        selection_batch_size = frame_length
    batch_starts = np.arange(0, frame_length, selection_batch_size)
    batch_ends = np.minimum(batch_starts + selection_batch_size, frame_length)
    selection_idxs = (np.random.randint(batch_starts.reshape(1, -1), batch_ends.reshape(1, -1), size=(num_poses, len(batch_starts))) 
                + np.arange(num_poses).reshape(-1, 1) * frame_length)
    selection_idxs = selection_idxs.flatten().tolist()
    return [dataset[idx] for idx in selection_idxs]

def scene_finetune(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    if stage == "coarse" and opt.zerostamp_init:
        raise NotImplementedError("coarse stage is not expected in finetuning")
        load_in_memory = True
        # batch_size = 4
    ...
    viewpoint_stack = sample_timewise_viewpoints(index_dataset, num_poses, frame_length, selection_batch_size=opt.camera_selection_batch_size)
    epoch_viewpoint_stack_cache = viewpoint_stack.copy()
    ...
    for iteration in range(first_iter, final_iter+1):        
        ...
        while idx < batch_size:
            if prepared_viewpoint_stack:
                viewpoint_cam = prepared_viewpoint_stack.pop(0)
            else:
                viewpoint_cam = viewpoint_stack.pop(randint(0,len(viewpoint_stack)-1))
                if not viewpoint_stack :
                    viewpoint_stack =  epoch_viewpoint_stack_cache.copy()
        ...
        for idx, viewpoint_cam in viewpoint_cams:
            render_pkg = render(viewpoint_cam, gaussians, pipe, background, stage=stage,cam_type=scene.dataset_type)
            image, viewspace_point_tensor, visibility_filter, radii = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"]
            images.append(image.unsqueeze(0))
            # gt_image = edit_cache.get_wait(idx).to(main_device)
            
            # gt_images.append(gt_image.unsqueeze(0))
            gt_image_idxs.append(idx)
            radii_list.append(radii.unsqueeze(0))
            visibility_filter_list.append(visibility_filter.unsqueeze(0))
            viewspace_point_tensor_list.append(viewspace_point_tensor)
        
        gt_images = edit_cache.get(gt_image_idxs)
        radii = torch.cat(radii_list,0).max(dim=0).values
        visibility_filter = torch.cat(visibility_filter_list).any(dim=0)
        image_tensor = torch.cat(images,0).to(main_device)
        gt_image_tensor = torch.stack(gt_images, 0).to(main_device).to(image_tensor.dtype)
        # gt_image_tensor = torch.cat(gt_images,0).to(main_device).to(image_tensor.dtype)
        # Loss
        # breakpoint()
        ...
        with torch.no_grad():
            ...
            if iteration % opt.iters_per_step == opt.iters_per_step - 1:
                viewpoint_stack = sample_timewise_viewpoints(index_dataset, num_poses, frame_length, selection_batch_size=opt.camera_selection_batch_size)
                epoch_viewpoint_stack_cache = viewpoint_stack.copy()
                prepared_viewpoint_stack = viewpoint_stack.copy()
                shuffle(prepared_viewpoint_stack)
                ...
                indexes = [idx for idx, _ in prepared_viewpoint_stack]
...
def training(dataset, hyper, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, expname):
    ...
    timer.start()
    # scene_finetune(dataset, opt, hyper, pipe, testing_iterations, saving_iterations,
    #                          checkpoint_iterations, checkpoint, debug_from,
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : EditScene, renderFunc, renderArgs, stage, dataset_type):
    if tb_writer:
